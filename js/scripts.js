...
const game = (doc, player1, player2, isSinglePlayerMode = false) => {
  ...
  modal.text = modal.modal.querySelector(".title");
...
const start = (() => {
  ...
  // game mode modal
  const modalGameMode = Modal(document, "#modal-game-mode");
  ...
  // form modal
  const modalForm = Modal(document, "#modal-form");
  ...
  function initGame(player1, player2, gameMode = false) {
    const g = game(document, player1, player2, gameMode);
    g.toggleShow();
  }
